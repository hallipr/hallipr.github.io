...
import { Point3D, RBush3D } from './rbush3d';
import { ClusterResult, cluster } from './clustering';
...
export class PointsWithIndex extends THREE.Points {
    ...
    cluster(epsilon: number, minPoints: number) {
        ...
        const clusterInfo: { clusterSize?: number, clusterCenter?: { x: number, y: number, z: number }, isNoise?: boolean }[] = [];
